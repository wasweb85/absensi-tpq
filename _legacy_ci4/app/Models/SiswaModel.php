...
class SiswaModel extends Model
{
   ...
   {
      $this->select('tb_siswa.*, tb_kelas.jilid, tb_kelas.index_kelas, tb_kelas.kategori, 
   ...
   public function getAllSiswaWithKelas($jilid = null, $kategori = null, $index = null, $search = null)
   {
      $query = $this->select('tb_siswa.*, tb_kelas.jilid, tb_kelas.index_kelas, tb_kelas.kategori, 
         CONCAT(tb_kelas.jilid, 
                IF(tb_kelas.kategori != "Campur", CONCAT(" ", tb_kelas.kategori), ""), 
                IF(tb_kelas.index_kelas != "", CONCAT(" ", tb_kelas.index_kelas), "")) as kelas')
         ->join(
      ...
      if (!empty($jilid)) {
         $query->where('tb_kelas.jilid', $jilid);
      }
      if (!empty($kategori)) {
         $query->where('tb_kelas.kategori', $kategori);
      }
      if (!empty($index)) {
   ...
   {
      return $this->select('tb_siswa.*, tb_kelas.jilid, tb_kelas.index_kelas, tb_kelas.kategori, 
         CONCAT(tb_kelas.jilid, 
                IF(tb_kelas.kategori != "Campur", CONCAT(" ", tb_kelas.kategori), ""), 
                IF(tb_kelas.index_kelas != "", CONCAT(" ", tb_kelas.index_kelas), "")) as kelas')
         ->join(
