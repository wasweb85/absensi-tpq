...
// >>>>>>>>>>ini code sebelum di tambah user siswa<<<<<<<<<<<<
...
// Admin
// Tambahkan ['filter' => 'login'] di sini 👇
$routes->group('admin', ['filter' => 'login'], function ($routes) {
   ...
   // Jadwal Pelajaran
   $routes->group('jadwal-pelajaran', ['namespace' => 'App\Controllers\Admin'], function ($routes) {
      $routes->get('/', 'JadwalPelajaranController::index');
      $routes->post('store', 'JadwalPelajaranController::store');
      $routes->post('update', 'JadwalPelajaranController::update');
      $routes->get('delete/(:num)', 'JadwalPelajaranController::delete/$1');
   });

   // Seragam
   $routes->group('seragam', ['namespace' => 'App\Controllers\Admin'], function ($routes) {
      $routes->get('/', 'SeragamController::index');
      $routes->post('store', 'SeragamController::store');
      $routes->post('update', 'SeragamController::update');
      $routes->get('delete/(:num)', 'SeragamController::delete/$1');
   });

   // Mata Pelajaran
   $routes->group('mapel', ['namespace' => 'App\Controllers\Admin'], function ($routes) {
      $routes->get('/', 'MapelController::index');
      $routes->post('store', 'MapelController::store');
      $routes->post('update', 'MapelController::update');
      $routes->get('delete/(:num)', 'MapelController::delete/$1');
   });
   ...
   $routes->group('backup', ['namespace' => 'App\Controllers\Admin'], function ($routes) {
      ...
      $routes->add('photos/restore', 'Backup::photosRestore');
   });
});
...
$routes->group('teacher', ['namespace' => 'App\Controllers\Teacher', 'filter' => 'login'], function (RouteCollection $routes) {
   ...
   // Absensi Manual (inline radio buttons)
   $routes->get('absen-manual', 'Dashboard::absenManual');
   $routes->post('absen-manual/save', 'Dashboard::saveManualAttendance');

   // Jadwal Pelajaran Mingguan
   $routes->get('jadwal', 'JadwalPelajaran::index');
});
// Group Route untuk Siswa
// Pastikan namanya 'authSiswa' (sesuai yang didaftarkan di Config)
$routes->group('siswa', ['filter' => 'authSiswa'], function ($routes) {
   ...
   // Riwayat Kehadiran (New Modern)
   $routes->get('riwayat', 'Siswa\Dashboard::riwayat');

   // Profil (New Modern)
   $routes->get('profil', 'Siswa\Dashboard::profil');
   
   // --- TAMBAHKAN BARIS INI UNTUK LOGOUT ---
   $routes->get('logout', 'AuthSiswa::logout'); 

});

// Route Login Siswa
$routes->get('login-siswa', 'AuthSiswa::index');
$routes->post('auth-siswa', 'AuthSiswa::login');
