...
class GenerateLaporan extends BaseController
{
   ...
   public function generateLaporanSiswa()
   {
      ...
      // jika mau hari sabtu dan minggu libur
      ...
      // }
      foreach ($period as $value) {
         // 1. KITA HAPUS IF YANG MEMBLOKIR HARI (Hapus if Sat/Sun tadi)
         // Biarkan semua hari lewat.

         $isLibur = ($value->format('D') == 'Tue' || $value->format('D') == 'Fri'); 

         // Tetap cek lewat hari ini atau belum
         $lewat = Time::parse($value->format('Y-m-d'))->isAfter(Time::today());
         ...
         // 3. SIMPAN STATUS LIBUR KE DALAM DATA
      ...
      if ($type == 'xls') {
         ...
         $this->response->setHeader(
            'Content-Disposition',
            'attachment;Filename=laporan_absen_' . $kelas['kelas'] . '_' . $begin->toLocalizedString('MMMM-Y') . '.xls'
         );
   ...
   public function generateLaporanGuru()
   {
      ...
      // >>>>>>>>untuk guru jika munggi dan sabtu libur<<<<<<
      ...
      foreach ($period as $value) {
         ...
         // 2. Cek apakah tanggal sudah lewat
         $lewat = Time::parse($value->format('Y-m-d'))->isAfter(Time::today());

         // 3. AMBIL DATA ABSENSI GURU (Perhatikan: Pakai presensiGuruModel)
         $absenByTanggal = $this->presensiGuruModel
            ->getPresensiByTanggal($value->format('Y-m-d'));

         $absenByTanggal['lewat'] = $lewat;

         // 4. Masukkan status libur ke dalam data
         $absenByTanggal['status_libur'] = $isLibur;

         // 5. Masukkan ke array utama
         array_push($dataAbsen, $absenByTanggal);
         array_push($arrayTanggal, Time::createFromInstance($value, locale: 'id'));
      }
      ...
      if ($type == 'xls') {
         ...
         $this->response->setHeader(
            'Content-Disposition',
            'attachment;Filename=laporan_absen_guru_' . $begin->toLocalizedString('MMMM-Y') . '.xls'
         );
