...
// =========================================================================
    ...
    public function getRiwayatSiswa($idSiswa, $waktu = 'bulan', $status = 'all')
    {
        ...
        $builder->select("
            tb_presensi_siswa.*,
            CASE 
                WHEN tb_presensi_siswa.id_kehadiran = 1 THEN 'Hadir'
                WHEN tb_presensi_siswa.id_kehadiran = 2 THEN 'Sakit'
                WHEN tb_presensi_siswa.id_kehadiran = 3 THEN 'Izin'
                WHEN tb_presensi_siswa.id_kehadiran = 4 THEN 'Tanpa Keterangan'
                ELSE 'Belum Absen'
            END as nama_kehadiran,
            CASE 
                WHEN tb_presensi_siswa.id_kehadiran = 1 THEN 'success'    /* Hijau */
                WHEN tb_presensi_siswa.id_kehadiran = 2 THEN 'warning'    /* Kuning */
                WHEN tb_presensi_siswa.id_kehadiran = 3 THEN 'info'       /* Biru */
                WHEN tb_presensi_siswa.id_kehadiran = 4 THEN 'danger'     /* Merah */
                ELSE 'secondary'                                          /* Abu-abu */
            END as warna
        ");
