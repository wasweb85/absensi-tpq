...
<span style="position: absolute;right: 0;">Kelas : <?= "{$kelas['kelas']}"; ?></span>
<table align="center" border="1">
   ...
   <?php foreach ($listSiswa as $siswa): ?>
      ...
      //>>>>>>>>>> jika hari sabtu dan minggu libur<<<<<<<<<<
      ...
      // }));
      $jumlahTidakHadir = count(array_filter($listAbsen, function ($a) use ($i) {
         // 1. CEK: JIKA LIBUR, JANGAN HITUNG SEBAGAI TIDAK HADIR
         if (!empty($a['status_libur'])) 
            return false;
      
         // Logika lama tetap jalan untuk hari biasa
         if ($a['lewat'])
            return false;
         if (is_null($a[$i]['id_kehadiran']) || $a[$i]['id_kehadiran'] == 4)
            return true;
      ...
      <tr>
         ...
         <?php foreach ($listAbsen as $absen): ?>
            <?php if (!empty($absen['status_libur'])): ?>
...
<table>
   ...
   <tr>
      <td>Laki-laki</td>
      <td>: <?= $rekapSiswa['laki']; ?></td>
   </tr>
   <tr>
      <td>Perempuan</td>
      <td>: <?= $rekapSiswa['perempuan']; ?></td>
   </tr>
</table>
<?php
